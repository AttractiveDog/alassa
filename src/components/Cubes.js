...
const Cubes = ({
  ...
  const onPointerMove = useCallback(
    e => {
      ...
      // Calculate position relative to cubes, even if mouse is outside bounds
      const colCenter = (e.clientX - rect.left) / cellW;
      const rowCenter = (e.clientY - rect.top) / cellH;

      // Only animate if within reasonable bounds of the grid
      if (colCenter >= -2 && colCenter <= gridSize + 2 && 
          rowCenter >= -2 && rowCenter <= gridSize + 2) {
  ...
  useEffect(() => {
    ...
    // Attach to document so it works even when hovering over content above cubes
